...
function Sidebar({
  ...
  return (
    <aside
      className={[
        // สูงเท่าหน้าจอเสมอ + sticky ไม่ให้ขยับเวลาหน้าเลื่อน
        "h-svh sticky top-0 left-0",
        // ไม่ให้ยืด/หดความกว้างตาม flex; ใช้ width ของตัวเองเท่านั้น
        "flex-none",
        // ผิว/เงา
        "flex flex-col bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/70",
        "border-r border-slate-200 shadow-[0_10px_30px_-15px_rgba(2,6,23,0.08)]",
        // transition แค่ width
    ...
    >
      ...
      {/* ----- MIDDLE: Navigation (เฉพาะส่วนเมนูที่สกอล์ได้ภายในไซด์บาร์) ----- */}
          ...
          {NAV.map((group, gi) => (
            <li key={gi}>
              ...
              <ul className="space-y-1">
                {group.items.map((it) => {
                  // active: เส้นทางตรงหรืออยู่ใต้เส้นทางนั้น
                  const active =
                    pathname === it.href ||
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    // จุดสำคัญ:
    // 1) ใช้ h-svh เพื่อปรับให้สูงเท่าหน้าจอจริงบนมือถือ
    // 2) overflow-hidden ที่คอนเทนเนอร์หลัก ป้องกันไม่ให้ body/page เลื่อน
    // 3) ให้ <main> เป็นตัวสกอล์แทน (overflow-y-auto)
    <div className="h-svh flex bg-slate-50 overflow-hidden">
      <Sidebar
        user={user}
        collapsed={collapsed}
      ...
      <main
        className={[
          "flex-1",
          // ทำให้คอนเทนต์เลื่อนในแกนตั้งได้ แต่ไม่กระทบ Sidebar
          "overflow-y-auto",
          // บางเคส Next/Image หรือ sticky header ภายใน main จะชอบมี spacing ด้านใน
          "min-w-0",
        ].join(" ")}
      >
        {children}
